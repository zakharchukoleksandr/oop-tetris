...
Что такое объект?
Объект - это сущность реального времени, имеющая определенное состояние и поведение. 
В Java Object - это экземпляр класса, имеющий переменные экземпляра как состояние 
объекта и методы как поведение объекта. Объект класса может быть создан с помощью 
ключевого слова new .
What is the difference between an object-oriented programming language and object-based 
programming language?
Существуют следующие основные различия между объектно-ориентированным языком и 
объектно-ориентированным языком.
o	Объектно-ориентированные языки следуют всем концепциям ООП, тогда как 
объектно-ориентированный язык не следует всем концепциям ООП, таким как наследование и 
полиморфизм.
o	Объектно-ориентированные языки не имеют встроенных объектов, тогда как 
объектно-ориентированные языки имеют встроенные объекты, например, JavaScript имеет объект
окна.
o	Примерами объектно-ориентированного программирования являются Java, C #, Smalltalk и т.
 Д., Тогда как примерами объектно-ориентированных языков являются JavaScript, VBScript и т. Д.
Каким будет начальное значение ссылки на объект, которая определяется как переменная 
экземпляра?
...
Конструктор можно определить как метод особого типа, который используется для инициализации 
состояния объекта. Он вызывается, когда создается экземпляр класса, и для объекта 
выделяется память. Каждый раз, когда объект создается с использованием ключевого 
слова new , вызывается конструктор класса по умолчанию. Имя конструктора должно 
быть похоже на имя класса. Конструктор не должен иметь явного возвращаемого типа.
В Java конструктор - это блок кода, похожий на метод. Он вызывается при создании 
экземпляра класса . Во время вызова конструктора память для объекта выделяется в памяти.
Это особый тип метода, который используется для инициализации объекта.
Каждый раз, когда объект создается с использованием ключевого слова new (), вызывается 
как минимум один конструктор.
Он вызывает конструктор по умолчанию, если в классе нет конструктора. В таком случае 
компилятор Java по умолчанию предоставляет конструктор по умолчанию.
В Java есть два типа конструкторов: конструктор без аргументов и параметризованный 
конструктор.
...
3.	Конструктор Java не может быть абстрактным, статическим, окончательным и 
синхронизированным.
Примечание: мы можем использовать модификаторы доступа при объявлении конструктора. 
Он контролирует создание объекта. Другими словами, у нас может быть частный, защищенный,
 общедоступный конструктор или конструктор по умолчанию в Java.
...
Конструктор по умолчанию используется для предоставления объекту значений по умолчанию, 
...
Параметризованный конструктор используется для предоставления разных значений отдельным
 объектам. Однако вы также можете указать те же значения.
...
Конструктор используется для инициализации состояния объекта.	Метод используется для 
раскрытия поведения объекта.
...
В Java нет конструктора копирования. Однако мы можем копировать значения из одного объекта 
в другой, как конструктор копирования в C ++.
...
Да, это текущий экземпляр класса (вы не можете использовать возвращаемый тип, но
 он возвращает значение).

Может ли конструктор выполнять другие задачи вместо инициализации?
Да, например, создание объекта, запуск потока, вызов метода и т. Д. Вы можете 
выполнять любую операцию в конструкторе так же, как и в методе.

Есть ли в Java класс конструктора?
Да.
________________________________________
Какова цель класса Constructor?
Java предоставляет класс конструктора, который можно использовать для получения внутренней
 информации конструктора в классе. Он находится в пакете java.lang.reflect.
 
Java static keyword
Статическое ключевое слово в Java используется для управления памятью главным образом.
 Мы можем применять ключевое слово static с переменными , методами, блоками и
 вложенными классами . Ключевое слово static принадлежит классу, а не экземпляру класса.
Статика может быть:
...
o	Статическая переменная может использоваться для ссылки на общее свойство всех объектов 
(которое не является уникальным для каждого объекта), например, название компании сотрудников, название колледжа студентов и т. Д.
o	Статическая переменная получает память только один раз в области класса во время загрузки
 класса.
...
5.	}  
Предположим, что в моем колледже 500 студентов, теперь все члены данных экземпляра будут 
получать память каждый раз при создании объекта. У всех студентов есть свое 
уникальное имя и номер, поэтому в этом случае подойдет элемент данных экземпляра. 
Здесь «колледж» относится к общей собственности всех объектов . Если мы сделаем его 
статическим, это поле получит память только один раз.
Статическое свойство Java является общим для всех объектов.

Статический метод Java
Если вы примените статическое ключевое слово с любым методом, это называется 
статическим методом.
...
Можем ли мы выполнить программу без метода main ()?
Ответ) Да, один из способов выполнить программу без основного метода - использовать
 статический блок.
...
byte можно повысить до short, int, long, float или double.
 ...
 double. Тип данных char может быть повышен до int, long, float
  ...
  Нет, мы не можем перегрузить методы, просто применив к ним ключевое слово static (количество параметров и типов одинаково). 

Можем ли мы перегрузить метод main ()?
Да, мы можем иметь любое количество основных методов в
 программе Java, используя перегрузку методов.
 
 Core Java - OOPs Concepts: Method Overriding Interview Questions
 Ядро Java - Концепции ООП: вопросы интервью Переопределение метода
 
 Что такое переопределение метода:
 Если подкласс предоставляет конкретную реализацию метода,
  который уже предоставлен его родительским классом, это 
  называется переопределением метода. Он используется для 
  полиморфизма во время выполнения и для реализации методов 
  интерфейса.
 Правила переопределения метода
 o	У метода должно быть то же имя, что и у родительского класса.
 o	Метод должен иметь ту же сигнатуру, что и в родительском классе.
 o	Два класса должны иметь между собой отношения IS-A.
 Можем ли мы переопределить статический метод?
 Нет, вы не можете переопределить статический метод, потому что 
 они являются частью класса, а не объекта.
 ________________________________________
 80) Почему мы не можем переопределить статический метод?
 Это связано с тем, что статический метод является частью класса
  и связан с классом, тогда как метод экземпляра связан с объектом, 
  а статический метод получает память в области класса, а экземпляр
   получает память в куче.
 ________________________________________
 81) Можем ли мы переопределить перегруженный метод?
 Да.
 Разница между перегрузкой метода и переопределением.
 Перегрузка метода	Переопределение метода
 1) Перегрузка метода увеличивает читаемость программы.	
 Переопределение метода обеспечивает
  конкретную реализацию метода,
  которая уже предоставлена его суперклассом.
 2) Перегрузка метода происходит внутри класса.	Переопределение 
 метода происходит в 
 двух классах, между которыми 
 существует связь IS-A.
 3) В этом случае параметры должны быть другими.	В этом случае 
 параметры должны
  быть одинаковыми.
 ________________________________________
 83) Можем ли мы переопределить частные методы?
 Нет, мы не можем переопределить частные методы, потому что объем 
 частных методов ограничен классом, и мы не можем получить к ним 
 доступ за пределами класса.
 ________________________________________
 84) Можем ли мы изменить объем переопределенного метода в подклассе?
 Да, мы можем изменить объем переопределенного метода в подклассе. 
 Однако мы должны заметить, что мы не можем уменьшить доступность 
 метода. При изменении доступности метода необходимо учитывать 
 следующий момент.
 o	Частное может быть изменено на защищенное, общедоступное или 
 значение по умолчанию.
 o	Защищенные можно изменить на общедоступные или по умолчанию.
 o	Значение по умолчанию можно изменить на общедоступное.
 o	Публика всегда будет публичной.
 ________________________________________
 85) Можем ли мы изменить предложение throws метода суперкласса, 
 переопределив его в подклассе?
 Да, мы можем изменить предложение throws метода суперкласса, 
 переопределив его в подклассе. Однако есть некоторые правила,
  которые необходимо соблюдать при отмене в случае обработки 
  исключений.
 o	Если метод суперкласса не объявляет исключение, метод 
 переопределения подкласса не может объявить проверенное исключение,
  но может объявить непроверенное исключение.
 ...
 Можете ли вы иметь виртуальные функции в Java?
 Да, все функции в Java по умолчанию виртуальные.
...
Как вы можете видеть в приведенном выше примере, тип возвращаемого значения метода get () класса A - A, но тип возвращаемого значения метода get () класса B - B. Оба метода имеют разный тип возвращаемого значения, но это метод переопределения. . Это называется ковариантным возвращаемым типом.
Как реализованы ковариантные возвращаемые типы?
Java не допускает перегрузки на основе возвращаемого типа, но JVM всегда допускает перегрузку на основе возвращаемого типа. JVM использует полную подпись метода для поиска / разрешения. Полная подпись означает, что она включает тип возвращаемого значения в дополнение к типам аргументов. т.е. класс может иметь два или более метода, различающихся только типом возвращаемого значения. javac использует этот факт для реализации ковариантных возвращаемых типов.

Ядро Java - Концепции ООП: заключительные вопросы собеседования
Core Java - OOPs Concepts: final keyword Interview Questions

Какая  final  переменная?
В Java последняя 	 final  используется, чтобы запретить 
пользователю обновлять ее. Если мы инициализируем последнюю 
переменную, мы не сможем изменить ее значение. 
Другими словами, мы можем сказать, что последняя переменная, 
однажды присвоенная значению, никогда не может быть изменена 
после этого. Последняя переменная, которой не присвоено 
какое-либо значение, может быть назначена только через 
конструктор класса.

FINAL ключевое слово в Java
...
2.	метод
3.	класс
Ключевое слово final может применяться с переменными, конечная 
переменная, не имеющая значения, называется пустой конечной 
переменной или неинициализированной конечной переменной. Его можно 
инициализировать только в конструкторе. Пустая конечная переменная 
также может быть статической, которая будет инициализирована только 
в статическом блоке. Мы подробно изучим их. Давайте сначала изучим 
основы ключевого слова final.

1) final переменная Java
Если вы сделаете какую-либо переменную final, вы не 
сможете изменить значение последней переменной 
(оно будет постоянным).

Java final method
последний класс Java
Если вы сделаете какой-либо класс final, вы не сможете его расширить.

Что такое пустая или неинициализированная final переменная?
Последняя переменная, которая не инициализирована во время объявления, называется пустой конечной переменной.

Можем ли мы инициализировать пустую final переменную?
Да, но только в конструкторе

Cтатическая пустая конечная переменная
Статическая конечная переменная, которая не инициализирована 
во время объявления, называется статической пустой конечной 
переменной. Его можно инициализировать только в статическом блоке.

Что такое конечный параметр?
Если вы объявите какой-либо параметр окончательным, вы не сможете изменить его значение.

Можно ли объявить конструктор окончательным?
Нет, потому что конструктор никогда не наследуется.

Какая последняя пустая переменная?
Последняя переменная, не инициализированная во время объявления, 
называется последней пустой переменной. Мы не можем напрямую 
инициализировать последнюю пустую переменную. Вместо этого мы 
должны инициализировать его с помощью конструктора класса. 
Это полезно в том случае, когда у пользователя есть одни данные, 
которые нельзя изменять другим, например, номер PAN. 

Можем ли мы инициализировать последнюю пустую переменную?
Да, если он не статический, мы можем инициализировать его в 
конструкторе. Если это статическая пустая конечная переменная,
 ее можно инициализировать только в статическом блоке.
Можно ли объявить основной метод окончательным?
Да, мы можем объявить основной метод как public static final 
void main (String [] args) {}.

98) Можно ли объявить конструктор финальным?
Конструктор никогда не может быть объявлен окончательным, 
потому что он никогда не наследуется. Конструкторы - это не 
обычные методы; поэтому объявлять конструкторы окончательными 
нет смысла. Однако, если вы попытаетесь это сделать, компилятор 
выдаст ошибку.
________________________________________
99) Можно ли объявить интерфейс окончательным?
Нет, мы не можем объявить интерфейс окончательным, потому что
 интерфейс должен быть реализован каким-то классом, чтобы 
 обеспечить его определение. Поэтому делать интерфейс окончательным
  нет смысла. Однако, если вы попытаетесь это сделать, компилятор 
  выдаст ошибку.
________________________________________
100) В чем разница между конечным методом и абстрактным методом?
Основное различие между методом final и абстрактным методом 
заключается в том, что абстрактный метод не может быть final, 
поскольку нам нужно переопределить их в подклассе, чтобы дать 
его определение.
